...
binompbar<- function(bu,alpha,beta,rho){ 
  ...
  # these are the parameters you'll need to specify when you call this function:
    ...
    # rho = temporal autocorrelation coefficient, falls between 0 and 1 for PBAR
  ...
  # We establish two beta distribution, U and W, with 2100 sampled values
  # See McKenzie 1985 for more information
  p=(rho*alpha*(alpha+beta))/(beta+rho*alpha) # p is calculated from alpha, beta and rho; slight modification from McKenzie's approach to makes things cleaner
  U=rbeta(2100, shape1=beta, shape2=alpha-p, ncp = 0)
  W=rbeta(2100, shape1=p, shape2=alpha-p, ncp = 0) 
  
  # U and W are combined, resulting in (if rho>0) a positively autocorrelated beta distribution bounded from 0 to 1
