...
# replica of 2010 Epidemiological Model of Virulence, CM Lively
# Here, there is no temporal variation in virulence
# the number of offspring of infected mothers is set at the beginning of the simulation


determ<- function(bu,bi){ # from Lively 2010
  #  THE FUNCTION
  ...
  # bi = number of offspring made by infected females - note we specify this here, it is not determined stochastically
  ...
  Wu[1]<-bu/(1+(au*N[1]))
  Wi[1]<-bi/(1+(ai*N[1])) # here bi is a fixed parameter
...
# 5 - selection event: realized birth rate
    # calculate realized number offspring (fitness), based upon population size at this time step.
    
    # we include these ifelse statements as additional way to ensure that birth rate stays above zero    if ((bu/(1+(au*N[i])))<0){
    if ((bu/(1+(au*N[i])))<0){
      Wu[i]<-0
    } else {
      Wu[i]<-(bu/(1+(au*N[i])))
